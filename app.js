...
async function exportToPDF() {
    ...
    doc.text('Résultat du RIATEST', pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 20;
    ...
    if (currentParticipantInfo) {
        if (currentParticipantInfo.prenom) {
            infoText += ` | Prénom: ${currentParticipantInfo.prenom}`;
        }
        infoText += ` | Âge: ${currentParticipantInfo.age} ans | Sexe: ${currentParticipantInfo.sex}`;
    }
    ...
    Object.keys(categories).forEach(category => {
        ...
        if (currentExcellentProfessions && currentExcellentProfessions[category] && currentExcellentProfessions[category].length > 0) {
            ...
            doc.setFontSize(10);
            const professionsText = `Métiers appréciés: ${currentExcellentProfessions[category].join(', ')}`;
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    const footerText = "Pour une interprétation approfondie de vos résultats et pour connaître les formations et métiers associés à votre profil, nous vous invitons à vous faire accompagner par un psyEN.";
...
function displayResults(percentages, password = null) {
    ...
    resultsDiv.innerHTML = '<h2>Résultats</h2>' +
        ...
        '<button onclick="exportToPDF()" style="background-color: #2196F3; color: white; padding: 10px 20px; border: none; border-radius: 4px; cursor: pointer; font-size: 14px;">📄 Exporter en PDF</button>' +
        '</div>' + passwordDisplay;
    ...
    resultsDiv.innerHTML += `
        <div class="exploration-section" style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0; border: 1px solid #dee2e6;">
            <h3 style="color: #495057; margin-bottom: 15px;">🔍 Explorez vos formations et métiers</h3>
            <p style="color: #6c757d; margin-bottom: 20px; line-height: 1.6;">
                Pour une interprétation approfondie de vos résultats et pour connaître les formations et métiers associés à votre profil RIASEC, nous vous invitons à vous faire accompagner par un psyEN.
                En attendant, vous pouvez explorer dès maintenant les formations et métiers qui correspondent à votre profil grâce à nos moteurs de recherche spécialisés.
            </p>
...
function displayRetrievedResults(percentages, responses) {
    ...
    const prenom = document.querySelector('.participant-summary')?.textContent?.match(/Prénom : ([^\n]+)/)?.[1]?.trim();
    const ageMatch = document.querySelector('.participant-summary')?.textContent?.match(/Âge : (\d+)/);
...
function openRIASECExplorer(type) {
    if (!currentPercentages) {
        alert('Aucun résultat disponible. Veuillez d\'abord passer le test.');
        return;
    }
